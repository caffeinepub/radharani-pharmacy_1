...
function generateDate(index: number): number {
  // Spread dates from Jan 2024 to April 2026 (~27 months = ~820 days)
  const startMs = new Date("2024-01-01").getTime();
  const endMs = new Date("2026-04-01").getTime();
  const range = endMs - startMs;
  // Use a quasi-random spread based on index
...
function pickRating(seed: number): number {
  const r = Math.abs(Math.sin(seed * 17.3)) * 100;
  if (r < 60) return 5;
  if (r < 85) return 4;
  if (r < 95) return 3;
  if (r < 98) return 2;
...
export const seededReviews: SeededReview[] = generateReviews(1500);
